...
public sealed class UexMcpTools(ProfilesConfig config, ProviderManager providers)
{
    ...
    [McpServerTool(Name = "preview_mesh"), Description("Export a mesh asset to a GLB file on disk and return the written path (read the file to view it).")]
    public string PreviewMesh(
        [Description("Game profile name")] string profile,
        [Description("Virtual asset path of the mesh package")] string asset,
        [Description("GLB output file path; default: a temp file")] string? outPath = null)
    {
        var provider = providers.Get(profile);
        var vpath = AssetOps.ResolvePackagePath(provider, asset);
        outPath ??= Path.Combine(Path.GetTempPath(), "uex",
            Path.ChangeExtension(vpath, null)!.Replace('/', '_') + ".glb");
        return AssetOps.MeshExportLine(AssetOps.SaveMeshGLB(provider, vpath, outPath), outPath);
    }
